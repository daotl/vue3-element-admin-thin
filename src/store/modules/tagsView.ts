export const useTagsViewStore = defineStore('tagsView', () => {
  ...
  function delView(view: TagView) {
    return new Promise<{ visitedViews: TagView[], cachedViews: string[] }>(
      (resolve, reject) => {
        Promise.all([delVisitedView(view), delCachedView(view)])
          .then(() => {
            resolve({
  ...
  function delOtherViews(view: TagView) {
    return new Promise<{ visitedViews: TagView[], cachedViews: string[] }>(
      (resolve, reject) => {
        Promise.all([delOtherVisitedViews(view), delOtherCachedViews(view)])
          .then(() => {
            resolve({
              visitedViews: [...visitedViews.value],
              cachedViews: [...cachedViews.value],
  ...
  function closeCurrentView() {
    ...
    void delView(tags).then((res: { visitedViews: TagView[] }) => {
      if (isActive(tags)) {
        void toLastView(res.visitedViews, tags)
      }
    })
  }
  ...
  function toLastView(visitedViews: TagView[], view?: TagView) {
    ...
    if (latestView && latestView.fullPath) {
      void router.push(latestView.fullPath)
    }
    else {
      ...
      if (view?.name === 'Dashboard') {
        // to reload home page
        void router.replace(`/redirect${view.fullPath}`)
      }
      else {
        void router.push('/')
      }
